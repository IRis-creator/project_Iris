...
total_attendance = {name: {"Present": 0, "Late Present": 0, "Absent": 0} for name in students}
...
# Function to update total attendance counts
def update_total_attendance(current_date):
    # Load the existing total attendance if it exists
    load_total_attendance()

    # Update the counts based on today's attendance
    ...
    # Write total attendance summary to a CSV file
...
# Clear previous attendance data for the current date if it exists
if os.path.exists(f'{current_date}_attendance.csv'):
    os.remove(f'{current_date}_attendance.csv')
...
# Update total attendance counts only for today
update_total_attendance(current_date)
