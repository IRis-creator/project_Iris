...
account_sid = 'SK9f69a244a62cca4f16560bb7fe424352q'  # Replace with your actual Twilio account SID
auth_token = 'QYVTtmGsj5ojX39d0teDiPOli5zyJ2Uw'    # Replace with your actual Twilio auth token
twilio_phone_number = '9864480833'  # Your Twilio phone number

client = Client(account_sid, auth_token)

# Dictionary mapping student names to their phone numbers
student_phone_numbers = {
    "John Doe": "+1234567890",  # Replace with actual student phone numbers
    "Aadarsha": 9765705491 # Add more students here
}
...
# Function to send a message to absent students using Twilio
def send_absent_message(name):
    if name in student_phone_numbers:
        message_body = f"Hi {name}, why are you absent?"
        try:
            message = client.messages.create(
                body=message_body,
                from_=twilio_phone_number,
                to=student_phone_numbers[name]
            )
            print(f"Sent message to {name}: {message_body}")
        except Exception as e:
